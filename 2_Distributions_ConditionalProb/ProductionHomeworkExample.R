...
if (interactive()){
  ...
  N = 10
  headcount_total_mov_average = moving_average(data_aggregate_day$HeadCount, N)
  ...
  boxplot(t(data_mean_dayofweek), main="Avg Headcount on Each Day of Week")
  ...
  loginfo((paste('The avg weekend headcount is', round(avg_weekend_hc,3),
                      '.  The avg week headcount is', round(avg_week_hc,3),'.')),
          logger="data_logger")
